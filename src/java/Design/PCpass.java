 ...
 * @author Dell
 */
public class PCpass extends HttpServlet {
       ...
       HttpSession session=request.getSession();
            ...
            if(pass.equals(db_pass))
            {
              if(npass.equals(cpass))  
              {
                  ...
                  if(i>0)
                  {
                      response.sendRedirect("PChngpass?msg=Password Updated Successfully");
                  }
              }
              else
              {
                  response.sendRedirect("PChngpass?msg=New Password Does Not Match");
              }
            }
            else
            {
                response.sendRedirect("PChngPass?msg=Incorrect Old Password");
            }
