 ...
 * @author Dell
 */
public class DCpass extends HttpServlet {
        ...
        try {
            ...
            if(pass.equals(db_pass))
            {
              if(npass.equals(cpass))  
              {
                  ...
                  if(i>0)
                  {
                      response.sendRedirect("DChngpass?msg=Password Updated Successfully");
                  }
              }
              else
              {
                  response.sendRedirect("DChngpass?msg=New Password Does Not Match");
              }
            }
            else
            {
                response.sendRedirect("DChngPass?msg=Incorrect Old Password");
            }
